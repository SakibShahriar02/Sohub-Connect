...
export default function EditTextToSpeech() {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const { updateTTS, getTTSById } = useTextToSpeech();
    ...
    const loadTTS = async () => {
      ...
      try {
        ...
        if (tts) {
          ...
          Swal.fire('Error', 'TTS entry not found', 'error');
          navigate('/voice/text-to-speech');
        }
      } catch (error) {
        Swal.fire('Error', 'Failed to load TTS entry', 'error');
        navigate('/voice/text-to-speech');
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await updateTTS(id, formData);
      Swal.fire('Success', 'TTS entry has been updated successfully', 'success');
      navigate('/voice/text-to-speech');
    } catch (error) {
      Swal.fire('Error', error instanceof Error ? error.message : 'Failed to update TTS entry', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <PageMeta title="Edit Text To Speech | Voice" description="Edit text to speech entry" />
